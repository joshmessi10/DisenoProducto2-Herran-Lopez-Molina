...
app.post('/register', [
    ...
    body('password').notEmpty().isStrongPassword( { minLength: 10, minLowercase: 1, minUppercase: 1, minNumbers: 1, minSymbols: 1, returnScore: false }).withMessage('La contraseña no es segura, debe tener al menos una mayúscula, minúscula, número y símbolo'),
    body('telefono').notEmpty().isMobilePhone('any', { strictMode: true }).withMessage('No es un número de teléfono'),
], async (req, res) => {
...
app.get("/v1/user", Verify, (req, res) => {
    res.status(200).json({
        ...
        user: req.user, // Puedes incluir información del usuario si lo deseas
    });
});
...
app.post('/evento', [
    ...
    body('geolocalizacion').notEmpty().withMessage('geolocalizacion es requerida')
], async (req, res) => {
    const errores = validationResult(req);
    if (!errores.isEmpty()) {
        return res.status(400).json({ errores: errores.array() });
